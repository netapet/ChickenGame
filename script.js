(() => {
  ...
  function drawFestival(t){
    ...
    for(let row=5;row>=0;row--){const z=.05+row*.13;
      ...
      for(const side of [-1,1]){const p=point(side*(.7+(row%2)*.08),z-.04),name=cast[(row+(side>0?2:0))%cast.length],img=sprites[name],height=(name==='chick2'?72:132)*p.s,bob=Math.abs(Math.sin(t*.005+row+side))*5*p.s;drawSprite(name,p.x,p.y-bob,height*(img.naturalWidth/img.naturalHeight||1),height,side<0);}
    }
